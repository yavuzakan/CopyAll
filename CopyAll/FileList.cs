...
namespace CopyAll
{
    public partial class FileList : Form
    {
        ...
        public void ara()
        {
            ...
            while (dr.Read())
            {
                dataGridView1.Rows.Insert(0, dr.GetValue(0).ToString(), dr.GetValue(4).ToString(), dr.GetValue(1).ToString(), dr.GetValue(2).ToString(), dr.GetValue(3).ToString(),  dr.GetValue(5).ToString());
            }
            con.Close();
        }
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                textBox1.Text = dataGridViewRow.Cells["file1"].Value.ToString();
